 ...
 *
 *   LIMITATIONS:
 *       - Most of the window/monitor functions are not implemented (not
 *required)
 *
 ...
 *           Reconfigure standard input to receive key inputs, works with SSH
 *connection. WARNING: Reconfiguring standard input could lead to undesired
 *effects, like breaking other running processes orblocking the device if not
 *restored properly. Use with care.
 *
 *   DEPENDENCIES:
 *       - DRM and GLM: System libraries for display initialization and
 *configuration
...
// To avoid conflict with the capturing code in rcore.c we undefine the macro
// KEY_F12, so the enum KEY_F12 from raylib is used
#undef KEY_F12
...
// Defines and Macros
//----------------------------------------------------------------------------------
#define USE_LAST_TOUCH_DEVICE // When multiple touchscreens are connected, only
                              // use the one with the highest event<N> number
...
typedef struct {
  ...
  // NOTE: currentButtonState[] can't be written directly due to multithreading,
  // app could miss the update
  char currentButtonStateEvdev[MAX_MOUSE_BUTTONS]; // Holds the new mouse state
  ...
  Rectangle
      absRange;  // Range of values for absolute pointing devices (touchscreens)
  int touchSlot; // Hold the touch slot number of the currently being sent
                 // multitouch block

  // Gamepad data
  int gamepadStreamFd[MAX_GAMEPADS]; // Gamepad device file descriptor
  int gamepadAbsAxisRange[MAX_GAMEPADS][MAX_GAMEPAD_AXIS]
                         [2]; // [0] = min, [1] = range value of the axis
  int gamepadAbsAxisMap[MAX_GAMEPADS]
                       [ABS_CNT]; // Maps the axes gamepads from the evdev api
                                  // to a sequential one
...
// NOTE: The complete evdev EV_KEY list can be found at
// /usr/include/linux/input-event-codes.h
// TODO: Complete the LUT with all unicode decimal values
// TODO: Replace this with a keymap from the X11 to get the correct regional map
// for the keyboard: Currently non US keyboards will have the wrong mapping for
// some keys NOTE: Replacing this with the keymap from X11 would probably be
// useless, as people use the drm backend to *avoid* X11
static const int evkeyToUnicodeLUT[] = {
    0,   27,  49,  50,  51,  52,  53,  54,  55,  56,  57,  48, 45, 61, 8,
    0,   113, 119, 101, 114, 116, 121, 117, 105, 111, 112, 0,  0,  13, 0,
    97,  115, 100, 102, 103, 104, 106, 107, 108, 59,  39,  96, 0,  92, 122,
    120, 99,  118, 98,  110, 109, 44,  46,  47,  0,   0,   0,  32
    // LUT currently incomplete, just mapped the most essential keys
};

// This is the map used to map any keycode returned from linux to a raylib code
// from 'raylib.h' NOTE: Use short here to save a little memory
static const short linuxToRaylibMap[KEYMAP_SIZE] = {
    // We don't map those with designated initialization, because we would
    // getting
...
static void
ConfigureEvdevDevice(char *device); // Identifies a input device and configures
                                    // it for use if appropriate
...
static int FindMatchingConnectorMode(
    const drmModeConnector *connector,
    const drmModeModeInfo
        *mode); // Search matching DRM mode in connector's mode list
static int FindExactConnectorMode(
    const drmModeConnector *connector, uint width, uint height, uint fps,
    bool allowInterlaced); // Search exactly matching DRM connector mode in
                           // connector's list
static int FindNearestConnectorMode(
    const drmModeConnector *connector, uint width, uint height, uint fps,
    bool allowInterlaced); // Search the nearest matching DRM connector mode in
                           // connector's list
...
int GetMonitorWidth(int monitor) {
  ...
  if (monitor != 0) {
    TRACELOG(LOG_WARNING,
             "GetMonitorWidth() implemented for first monitor only");
...
int GetMonitorHeight(int monitor) {
  ...
  if (monitor != 0) {
    TRACELOG(LOG_WARNING,
             "GetMonitorHeight() implemented for first monitor only");
  } else if ((platform.connector) && (platform.modeIndex >= 0)) {
...
int GetMonitorPhysicalWidth(int monitor) {
  ...
  if (monitor != 0) {
    TRACELOG(LOG_WARNING,
             "GetMonitorPhysicalWidth() implemented for first monitor only");
  } else if ((platform.connector) && (platform.modeIndex >= 0)) {
...
int GetMonitorPhysicalHeight(int monitor) {
  ...
  if (monitor != 0) {
    TRACELOG(LOG_WARNING,
             "GetMonitorPhysicalHeight() implemented for first monitor only");
  } else if ((platform.connector) && (platform.modeIndex >= 0)) {
...
const char *GetMonitorName(int monitor) {
  ...
  if (monitor != 0) {
    TRACELOG(LOG_WARNING,
             "GetMonitorName() implemented for first monitor only");
  } else if ((platform.connector) && (platform.modeIndex >= 0)) {
...
double GetTime(void) {
  ...
  time = (double)(nanoSeconds - CORE.Time.base) *
         1e-9; // Elapsed time since InitTimer()
...
void PollInputEvents(void) {
...
#if defined(SUPPORT_SSH_KEYBOARD_RPI)
  // NOTE: Keyboard reading could be done using input_event(s) or just read from
  // stdin, both methods are used here. stdin reading is still used for legacy
  // purposes, it allows keyboard input trough SSH console
  ...
  for (int i = 0; i < MAX_MOUSE_BUTTONS; i++) {
    ...
    CORE.Input.Mouse.currentButtonState[i] =
        platform.currentButtonStateEvdev[i];
...
int InitPlatform(void) {
  ...
  platform.fd = open("/dev/dri/by-path/platform-gpu-card",
                     O_RDWR); // VideoCore VI (Raspberry Pi 4)
  ...
  if ((platform.fd == -1) || (drmModeGetResources(platform.fd) == NULL)) {
    ...
    platform.fd =
        open("/dev/dri/card0", O_RDWR); // VideoCore IV (Raspberry Pi 1-3)
  }
#endif
  ...
  for (size_t i = 0; i < res->count_connectors; i++) {
    ...
    drmModeConnector *con =
        drmModeGetConnector(platform.fd, res->connectors[i]);
    TRACELOG(LOG_TRACE, "DISPLAY: Connector modes detected: %i",
             con->count_modes);

    // In certain cases the status of the conneciton is reported as UKNOWN, but
    // it is still connected. This might be a hardware or software limitation
    // like on Raspberry Pi Zero with composite output.
  ...
  drmModeEncoder *enc =
      drmModeGetEncoder(platform.fd, platform.connector->encoder_id);
  if (!enc) {
  ...
  // If InitWindow should use the current mode find it in the connector's mode
  // list
  if ((CORE.Window.screen.width <= 0) || (CORE.Window.screen.height <= 0)) {
    ...
    platform.modeIndex =
        FindMatchingConnectorMode(platform.connector, &platform.crtc->mode);
  ...
  platform.modeIndex =
      FindExactConnectorMode(platform.connector, CORE.Window.screen.width,
                             CORE.Window.screen.height, fps, allowInterlaced);

  // If nothing found, try to find a nearly matching mode
  if (platform.modeIndex < 0)
    platform.modeIndex = FindNearestConnectorMode(
        platform.connector, CORE.Window.screen.width, CORE.Window.screen.height,
        fps, allowInterlaced);
  ...
  if (platform.modeIndex < 0)
    platform.modeIndex =
        FindExactConnectorMode(platform.connector, CORE.Window.screen.width,
                               CORE.Window.screen.height, fps, true);

  // If nothing found, try to find a nearly matching mode including interlaced
  if (platform.modeIndex < 0)
    platform.modeIndex =
        FindNearestConnectorMode(platform.connector, CORE.Window.screen.width,
                                 CORE.Window.screen.height, fps, true);

  // If nothing found, there is no suitable mode
  if (platform.modeIndex < 0) {
    TRACELOG(LOG_WARNING,
             "DISPLAY: Failed to find a suitable DRM connector mode");
    drmModeFreeEncoder(enc);
    drmModeFreeResources(res);
    return -1;
  }
  ...
  TRACELOG(LOG_INFO, "DISPLAY: Selected DRM connector mode %s (%ux%u%c@%u)",
           platform.connector->modes[platform.modeIndex].name,
           platform.connector->modes[platform.modeIndex].hdisplay,
           platform.connector->modes[platform.modeIndex].vdisplay,
           (platform.connector->modes[platform.modeIndex].flags &
  ...
  platform.gbmSurface = gbm_surface_create(
      platform.gbmDevice,
      platform.connector->modes[platform.modeIndex].hdisplay,
      platform.connector->modes[platform.modeIndex].vdisplay,
  ...
  const EGLint framebufferAttribs[] = {
      EGL_RENDERABLE_TYPE,
      (rlGetVersion() == RL_OPENGL_ES_30)
          ? EGL_OPENGL_ES3_BIT
          : EGL_OPENGL_ES2_BIT,         // Type of context support
      ...
      // EGL_TRANSPARENT_TYPE, EGL_NONE, // Request transparent framebuffer
      // (EGL_TRANSPARENT_RGB does not work on RPI)
  ...
  if (eglInitialize(platform.device, NULL, NULL) == EGL_FALSE) {
    // If all of the calls to eglInitialize returned EGL_FALSE then an error has
    // occurred.
    TRACELOG(LOG_WARNING, "DISPLAY: Failed to initialize EGL device");
    return -1;
  }
  ...
  if (!eglChooseConfig(platform.device, NULL, NULL, 0, &numConfigs)) {
    TRACELOG(LOG_WARNING, "DISPLAY: Failed to get EGL config count: 0x%x",
             eglGetError());
    return -1;
  }
  ...
  if (!eglChooseConfig(platform.device, framebufferAttribs, configs, numConfigs,
                       &matchingNumConfigs)) {
    TRACELOG(LOG_WARNING, "DISPLAY: Failed to choose EGL config: 0x%x",
             eglGetError());
    free(configs);
    return -1;
  }

  TRACELOG(LOG_TRACE, "DISPLAY: EGL matching configs available: %d",
           matchingNumConfigs);
  ...
  for (EGLint i = 0; i < matchingNumConfigs; ++i) {
    EGLint id = 0;
    if (!eglGetConfigAttrib(platform.device, configs[i], EGL_NATIVE_VISUAL_ID,
                            &id)) {
      TRACELOG(LOG_WARNING, "DISPLAY: Failed to get EGL config attribute: 0x%x",
               eglGetError());
      continue;
    }
  ...
  platform.context = eglCreateContext(platform.device, platform.config,
  ...
  platform.surface =
      eglCreateWindowSurface(platform.device, platform.config,
                             (EGLNativeWindowType)platform.gbmSurface, NULL);
  if (EGL_NO_SURFACE == platform.surface) {
    TRACELOG(LOG_WARNING,
             "DISPLAY: Failed to create EGL window surface: 0x%04x",
             eglGetError());
    return -1;
  }
  ...
  EGLBoolean result = eglMakeCurrent(platform.device, platform.surface,
                                     platform.surface, platform.context);
...
void ClosePlatform(void) {
  ...
  if (platform.crtc) {
    if (platform.connector) {
      drmModeSetCrtc(platform.fd, platform.crtc->crtc_id,
                     platform.crtc->buffer_id, platform.crtc->x,
                     platform.crtc->y, &platform.connector->connector_id, 1,
                     &platform.crtc->mode);
  ...
  CORE.Window.shouldClose =
      true; // Added to force threads to exit when the close window is called
...
static void InitKeyboard(void) {
  // NOTE: We read directly from Standard Input (stdin) - STDIN_FILENO file
  // descriptor, Reading directly from stdin will give chars already key-mapped
  // by kernel to ASCII or UNICODE
  ...
  // New terminal settings for keyboard: turn off buffering (non-canonical
  // mode), echo and key processing NOTE: ISIG controls if ^C and ^Z generate
  // break signals or not
  keyboardNewSettings.c_lflag &= ~(ICANON | ECHO | ISIG);
  // keyboardNewSettings.c_iflag &= ~(ISTRIP | INLCR | ICRNL | IGNCR | IXON |
  // IXOFF);
  ...
  platform.defaultFileFlags =
      fcntl(STDIN_FILENO, F_GETFL,
            0); // F_GETFL: Get the file access mode and the file status flags
  fcntl(STDIN_FILENO, F_SETFL,
        platform.defaultFileFlags | O_NONBLOCK); // F_SETFL: Set the file status
                                                 // flags to the value specified

  // NOTE: If ioctl() returns -1, it means the call failed for some reason
  // (error code set in errno)
  ...
  if (result < 0)
    TRACELOG(LOG_WARNING, "DRM: Failed to change keyboard mode, an SSH "
                          "keyboard is probably used");
  else {
...
#define MAX_KEYBUFFER_SIZE 32 // Max size in bytes to read
  ...
  bufferByteCount =
      read(STDIN_FILENO, keysBuffer, MAX_KEYBUFFER_SIZE); // POSIX system call
  ...
  for (int i = 0; i < bufferByteCount; i++) {
    // NOTE: If (key == 0x1b), depending on next key, it could be a special
    // keymap code! Up -> 1b 5b 41 / Left -> 1b 5b 44 / Right -> 1b 5b 43 / Down
    // -> 1b 5b 42
    if (keysBuffer[i] == 0x1b) {
      ...
      if (bufferByteCount == 1)
        ...
        {
          if ((keysBuffer[i + 2] == 0x5b) || (keysBuffer[i + 2] == 0x31) ||
              (keysBuffer[i + 2] == 0x32)) {
    ...
    } else if (keysBuffer[i] ==
               0x0a) // raylib KEY_ENTER (don't mix with <linux/input.h> KEY_*)
    {
...
static void InitEvdevInput(void) {
  ...
  if (directory) {
    while ((entity = readdir(directory)) != NULL) {
      if ((strncmp("event", entity->d_name, strlen("event")) ==
           0) || // Search for devices named "event*"
          (strncmp("mouse", entity->d_name, strlen("mouse")) ==
           0)) // Search for devices named "mouse*"
      {
    ...
    TRACELOG(LOG_WARNING, "INPUT: Failed to open linux event directory: %s",
             DEFAULT_EVDEV_PATH);
}
...
#define TEST_BIT(array, bit) ((array[LONG(bit)] >> OFF(bit)) & 1)
  ...
  // At this point we have a connection to the device, but we don't yet know
  // what the device is. It could be many things, even as simple as a power
  // button...
  //-------------------------------------------------------------------------------------------------------
  ...
  // Some devices could have properties we assosciate with keyboards as well as
  // properties we assosciate with mice
  ...
  if (TEST_BIT(evBits, EV_ABS)) {
    ...
    // If the device has an X an Y axis it's either a touch device, a special
    // mouse or a gamepad
    ...
    if (hasAbsXY &&
        (TEST_BIT(keyBits, BTN_STYLUS) || TEST_BIT(keyBits, BTN_TOOL_PEN) ||
    ...
    else if (hasAbsXY && TEST_BIT(keyBits, BTN_MOUSE))
      isMouse = true;
  ...
  if (TEST_BIT(evBits, EV_REL)) {
    ...
    // If it has any of the gamepad or touch features we tested so far, it's not
    // a mouse
    if (!isTouch && !isGamepad && TEST_BIT(relBits, REL_X) &&
        TEST_BIT(relBits, REL_Y) && TEST_BIT(keyBits, BTN_MOUSE))
      isMouse = true;
  }
  ...
  if (TEST_BIT(evBits, EV_KEY)) {
    ...
    // If we find any of the common gamepad buttons we assume it's a gamepad
      ...
      for (int button = BTN_JOYSTICK; button < BTN_DIGI; ++button) {
        if (TEST_BIT(keyBits, button))
          isGamepad = true;
      }

      for (int button = BTN_TRIGGER_HAPPY1; button <= BTN_TRIGGER_HAPPY40;
           button++) {
        if (TEST_BIT(keyBits, button))
          isGamepad = true;
      }
  ...
  if (isMouse || isTouch) {
    ...
    if (absAxisCount > 0) {
      ...
      platform.absRange.width =
          absinfo[ABS_X].info.maximum - absinfo[ABS_X].info.minimum;

      platform.absRange.y = absinfo[ABS_Y].info.minimum;
      platform.absRange.height =
          absinfo[ABS_Y].info.maximum - absinfo[ABS_Y].info.minimum;
    }
  } else if (isGamepad && !isMouse && !isKeyboard &&
             (platform.gamepadCount < MAX_GAMEPADS)) {
    ...
    ioctl(platform.gamepadStreamFd[index], EVIOCGNAME(64),
          &CORE.Input.Gamepad.name[index]);
    CORE.Input.Gamepad.axisCount[index] = absAxisCount;

    if (absAxisCount > 0) {
      // TODO / NOTE
      // So gamepad axis (as in the actual linux joydev.c) are just simply
      // enumerated and (at least for some input drivers like xpat) it's
      // convention to use ABS_X, ABX_Y for one joystick ABS_RX, ABS_RY for the
      // other and the Z axes for the shoulder buttons If these are now
      // enumerated you get LJOY_X, LJOY_Y, LEFT_SHOULDERB, RJOY_X, ... That
      // means they don't match the GamepadAxis enum This could be fixed
      int axisIndex = 0;
      for (int axis = ABS_X; axis < ABS_PRESSURE; axis++) {
        if (absinfo[axis].exist) {
          platform.gamepadAbsAxisRange[index][axisIndex][0] =
              absinfo[axisIndex].info.minimum;
          platform.gamepadAbsAxisRange[index][axisIndex][1] =
              absinfo[axisIndex].info.maximum - absinfo[axisIndex].info.minimum;
  ...
  TRACELOG(LOG_INFO, "INPUT: Initialized input device %s as %s", device,
           deviceKindStr);
}
  ...
  while (read(fd, &event, sizeof(event)) == (int)sizeof(event)) {
...
#if defined(SUPPORT_SSH_KEYBOARD_RPI)
    // If the event was a key, we know a working keyboard is connected, so
    // disable the SSH keyboard
    ...
    if ((event.code >= 1) && (event.code <= 255)) {
      ...
      if ((keycode > 0) && (keycode < MAX_KEYBOARD_KEYS)) {
        ...
        // Event interface: 'value' is the value the event carries. Either a
        // relative change for EV_REL, absolute new value for EV_ABS (joysticks
        // ...), or 0 for EV_KEY for release, 1 for keypress and 2 for
        // autorepeat
        ...
        if (event.value == 1) {
          if (CORE.Input.Keyboard.keyPressedQueueCount <
          ...
          if (CORE.Input.Keyboard.charPressedQueueCount <
              MAX_CHAR_PRESSED_QUEUE) {
            // TODO/FIXME: This is not actually converting to unicode properly
            // because it's not taking things like shift into account
            CORE.Input.Keyboard
                .charPressedQueue[CORE.Input.Keyboard.charPressedQueueCount] =
                evkeyToUnicodeLUT[event.code];
            CORE.Input.Keyboard.charPressedQueueCount++;
          }
        }

        TRACELOG(LOG_DEBUG,
                 "INPUT: KEY_%s Keycode(linux): %4i KeyCode(raylib): %4i",
                 (event.value == 0) ? "UP  " : "DOWN", event.code, keycode);
      }
    }
  }
}
...
static void PollGamepadEvents(void) {
  ...
  for (int i = 0; i < platform.gamepadCount; i++) {
    ...
    while (read(platform.gamepadStreamFd[i], &event, sizeof(event)) ==
           (int)sizeof(event)) {
      if (event.type == EV_KEY) {
        if (event.code < KEYMAP_SIZE) {
          ...
          TRACELOG(LOG_DEBUG,
                   "INPUT: Gamepad %2i: KEY_%s Keycode(linux): %4i "
                   "Keycode(raylib): %4i",
                   i, (event.value == 0) ? "UP" : "DOWN", event.code,
                   keycodeRaylib);

          if ((keycodeRaylib != 0) && (keycodeRaylib < MAX_GAMEPAD_BUTTONS)) {
            // 1 - button pressed, 0 - button released
            CORE.Input.Gamepad.currentButtonState[i][keycodeRaylib] =
                event.value;

            CORE.Input.Gamepad.lastButtonPressed =
                (event.value == 1) ? keycodeRaylib : GAMEPAD_BUTTON_UNKNOWN;
          }
        }
        ...
        if (event.code < ABS_CNT) {
          ...
          TRACELOG(LOG_DEBUG, "INPUT: Gamepad %2i: Axis: %2i Value: %i", i,
                   axisRaylib, event.value);
          ...
          if (axisRaylib < MAX_GAMEPAD_AXIS) {
            ...
            CORE.Input.Gamepad.axisState[i][axisRaylib] =
                (2 * (float)(event.value - min) / range) - 1;
          }
        }
      }
    }
...
static void PollMouseEvents(void) {
  ...
  // Try to read data from the mouse/touch/gesture and only continue if
  // successful
  while (read(fd, &event, sizeof(event)) == (int)sizeof(event)) {
    ...
    if (event.type == EV_REL) {
      ...
      if (event.code == REL_WHEEL)
        platform.eventWheelMove.y += event.value;
    }
    ...
    if (event.type == EV_ABS) {
      // Basic movement
      if (event.code == ABS_X) {
        CORE.Input.Mouse.currentPosition.x =
            (event.value - platform.absRange.x) * CORE.Window.screen.width /
            platform.absRange.width; // Scale according to absRange
        CORE.Input.Touch.position[0].x =
            (event.value - platform.absRange.x) * CORE.Window.screen.width /
            platform.absRange.width; // Scale according to absRange
      ...
      if (event.code == ABS_Y) {
        CORE.Input.Mouse.currentPosition.y =
            (event.value - platform.absRange.y) * CORE.Window.screen.height /
            platform.absRange.height; // Scale according to absRange
        CORE.Input.Touch.position[0].y =
            (event.value - platform.absRange.y) * CORE.Window.screen.height /
            platform.absRange.height; // Scale according to absRange
      ...
      if (event.code == ABS_MT_SLOT)
        platform.touchSlot =
            event.value; // Remember the slot number for the folowing events

      if (event.code == ABS_MT_POSITION_X) {
        if (platform.touchSlot < MAX_TOUCH_POINTS)
          CORE.Input.Touch.position[platform.touchSlot].x =
              (event.value - platform.absRange.x) * CORE.Window.screen.width /
              platform.absRange.width; // Scale according to absRange
      }
      ...
      if (event.code == ABS_MT_POSITION_Y) {
        if (platform.touchSlot < MAX_TOUCH_POINTS)
          CORE.Input.Touch.position[platform.touchSlot].y =
              (event.value - platform.absRange.y) * CORE.Window.screen.height /
              platform.absRange.height; // Scale according to absRange
      }
      ...
      if (event.code == ABS_PRESSURE) {
        int previousMouseLeftButtonState =
            platform.currentButtonStateEvdev[MOUSE_BUTTON_LEFT];
    ...
    if (event.type == EV_KEY) {
      ...
      if (event.code == BTN_RIGHT)
        platform.currentButtonStateEvdev[MOUSE_BUTTON_RIGHT] = event.value;
      if (event.code == BTN_MIDDLE)
        platform.currentButtonStateEvdev[MOUSE_BUTTON_MIDDLE] = event.value;
      if (event.code == BTN_SIDE)
        platform.currentButtonStateEvdev[MOUSE_BUTTON_SIDE] = event.value;
      if (event.code == BTN_EXTRA)
        platform.currentButtonStateEvdev[MOUSE_BUTTON_EXTRA] = event.value;
      if (event.code == BTN_FORWARD)
        platform.currentButtonStateEvdev[MOUSE_BUTTON_FORWARD] = event.value;
      if (event.code == BTN_BACK)
        platform.currentButtonStateEvdev[MOUSE_BUTTON_BACK] = event.value;
    }

    // Screen confinement
    if (!CORE.Input.Mouse.cursorHidden) {
      if (CORE.Input.Mouse.currentPosition.x < 0)
        CORE.Input.Mouse.currentPosition.x = 0;
      if (CORE.Input.Mouse.currentPosition.x >
          CORE.Window.screen.width / CORE.Input.Mouse.scale.x)
        CORE.Input.Mouse.currentPosition.x =
            CORE.Window.screen.width / CORE.Input.Mouse.scale.x;

      if (CORE.Input.Mouse.currentPosition.y < 0)
        CORE.Input.Mouse.currentPosition.y = 0;
      if (CORE.Input.Mouse.currentPosition.y >
          CORE.Window.screen.height / CORE.Input.Mouse.scale.y)
        CORE.Input.Mouse.currentPosition.y =
            CORE.Window.screen.height / CORE.Input.Mouse.scale.y;
    }
    ...
    for (int i = 0; i < MAX_TOUCH_POINTS; i++) {
      if (CORE.Input.Touch.position[i].x >= 0)
...
#define BINCMP(a, b)                                                           \
  memcmp((a), (b), (sizeof(a) < sizeof(b)) ? sizeof(a) : sizeof(b))

  for (size_t i = 0; i < connector->count_modes; i++) {
    TRACELOG(LOG_TRACE, "DISPLAY: DRM mode: %d %ux%u@%u %s", i,
    ...
    if (0 == BINCMP(&platform.crtc->mode, &platform.connector->modes[i]))
      return i;
  }
...
static int FindExactConnectorMode(const drmModeConnector *connector, uint width,
                                  uint height, uint fps, bool allowInterlaced) {
  TRACELOG(LOG_TRACE,
           "DISPLAY: Searching exact connector mode for %ux%u@%u, selecting an "
           "interlaced mode is allowed: %s",
           width, height, fps, allowInterlaced ? "yes" : "no");
  ...
  for (int i = 0; i < platform.connector->count_modes; i++) {
    ...
    TRACELOG(LOG_TRACE, "DISPLAY: DRM Mode %d %ux%u@%u %s", i, mode->hdisplay,
...
static int FindNearestConnectorMode(const drmModeConnector *connector,
                                    uint width, uint height, uint fps,
                                    bool allowInterlaced) {
  TRACELOG(LOG_TRACE,
           "DISPLAY: Searching nearest connector mode for %ux%u@%u, selecting "
           "an interlaced mode is allowed: %s",
           width, height, fps, allowInterlaced ? "yes" : "no");
  ...
  for (int i = 0; i < platform.connector->count_modes; i++) {
    ...
    const int nearestWidthDiff =
        abs(platform.connector->modes[nearestIndex].hdisplay - width);
    const int nearestHeightDiff =
        abs(platform.connector->modes[nearestIndex].vdisplay - height);
    const int nearestFpsDiff =
        abs(platform.connector->modes[nearestIndex].vrefresh - fps);

    if ((widthDiff < nearestWidthDiff) || (heightDiff < nearestHeightDiff) ||
        (fpsDiff < nearestFpsDiff))
      nearestIndex = i;
  }
