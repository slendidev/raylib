...
#define SDL_ENABLE_OLD_NAMES // Just in case we're on SDL3, we need some
                             // in-between compatibily
#endif
...
#define MAX_CLIPBOARD_BUFFER_LENGTH                                            \
  1024 // Size of the clipboard buffer used on GetClipboardText()
#endif

#if ((defined(SDL_MAJOR_VERSION) && (SDL_MAJOR_VERSION == 3)) &&               \
     (defined(SDL_MINOR_VERSION) && (SDL_MINOR_VERSION >= 1)))
...
// SDL3 Migration: SDL_INIT_TIMER - no longer needed before calling
// SDL_AddTimer()
#define SDL_INIT_TIMER                                                         \
  0x0 // It's a flag, so no problem in setting it to zero if we use in a bitor
      // (|)

// SDL3 Migration: The SDL_WINDOW_SHOWN flag has been removed. Windows are shown
// by default and can be created hidden by using the SDL_WINDOW_HIDDEN flag.
#define SDL_WINDOW_SHOWN                                                       \
  0x0 // It's a flag, so no problem in setting it to zero if we use in a bitor
      // (|)

// SDL3 Migration: Renamed
// IMPORTANT: Might need to call SDL_CleanupEvent somewhere see
// :https://github.com/libsdl-org/SDL/issues/3540#issuecomment-1793449852
...
const char *SDL_GameControllerNameForIndex(int joystickIndex) {
  // NOTE: SDL3 uses the IDs itself (SDL_JoystickID) instead of SDL2
  // joystick_index
...
// SLD3 Migration: To emulate SDL2 this function should return `SDL_DISABLE` or
// `SDL_ENABLE` representing the *processing state* of the event before this
// function makes any changes to it
...
void SDL_GetCurrentDisplayMode_Adapter(SDL_DisplayID displayID,
...
//     SDL_GetWindowDisplayScale() times 160 on iPhone and Android, and 96 on
//     other platforms.
...
void *SDL_GetClipboardData(const char *mime_type, size_t *size) {
  TRACELOG(LOG_WARNING,
           "Getting clipboard data that is not text is only available in SDL3");

  // We could possibly implement it ourselves in this case for some easier
  // platforms
  return NULL;
}
...
#if defined(PLATFORM_DESKTOP_SDL3) // SDL3 Migration: Monitor is an id instead
                                   // of index now, returns 0 on failure
...
void SetWindowIcon(Image image) {
  ...
  case PIXELFORMAT_UNCOMPRESSED_R16G16B16A16:
    ...
    break;
  default:
    return; // Compressed formats are not supported
  }
...
void SetWindowMonitor(int monitor) {
  ...
  if ((monitor >= 0) && (monitor < monitorCount)) {
    ...
    // 2. A workaround for SDL2 is leaving fullscreen, moving the window, then
...
#endif
    {
      ...
      if ((screenWidth >= usableBounds.w) || (screenHeight >= usableBounds.h)) {
        ...
        // 3. It wasn't done here because we can't assume changing the window
        // size automatically
...
Vector2 GetWindowScaleDPI(void) {
  ...
  // TRACELOG(LOG_WARNING, "GetWindowScaleDPI() not implemented on target
  // platform");
#else
...
Image GetClipboardImage(void) {
  ...
  for (int i = 0; i < SDL_arraysize(imageFormats); ++i) {
    ...
    if (fileData) {
      ...
      if (IsImageValid(image)) {
        TRACELOG(
            LOG_INFO,
            "Clipboard image: Got image from clipboard as a `%s` successfully",
            imageExtensions[i]);
        return image;
      }
    }
  ...
  if (!IsImageValid(image))
    TRACELOG(LOG_WARNING,
             "Clipboard image: Couldn't get clipboard data. Error: %s",
             SDL_GetError());
#endif
...
void SetGamepadVibration(int gamepad, float leftMotor, float rightMotor,
  ...
  if ((gamepad < MAX_GAMEPADS) && CORE.Input.Gamepad.ready[gamepad] &&
    ...
    if (rightMotor > 1.0f)
      rightMotor = 1.0f;
    if (duration > MAX_GAMEPAD_VIBRATION_TIME)
      duration = MAX_GAMEPAD_VIBRATION_TIME;

    SDL_GameControllerRumble(
        platform.gamepad[gamepad], (Uint16)(leftMotor * 65535.0f),
...
const char *GetKeyName(int key) { return SDL_GetKeyName(key); }
...
void PollInputEvents(void) {
  ...
  bool realTouch =
      false; // Flag to differentiate real touch gestures from mouse ones
  ...
  */
  ...
  if ((CORE.Window.eventWaiting) ||
      (((CORE.Window.flags & FLAG_WINDOW_MINIMIZED) > 0) &&
       ((CORE.Window.flags & FLAG_WINDOW_ALWAYS_RUN) == 0))) {
    SDL_WaitEvent(NULL);
  ...
  while (SDL_PollEvent(&event) != 0) {
...
#if defined(PLATFORM_DESKTOP_SDL3)
        // const char *data;   /**< The text for SDL_EVENT_DROP_TEXT and the
        // file name for SDL_EVENT_DROP_FILE, NULL for other events */ Event
        // memory is now managed by SDL, so you should not free the data in
        // SDL_EVENT_DROP_FILE, and if you want to hold onto the text in
        // SDL_EVENT_TEXT_EDITING and SDL_EVENT_TEXT_INPUT events, you should
        // make a copy of it. SDL_TEXTINPUTEVENT_TEXT_SIZE is no longer
    ...
    //     In general, handling this change just means checking for the
    //     individual events instead of first checking for SDL_WINDOWEVENT and
    //     then checking for window events. You can compare the event >=
    //     SDL_EVENT_WINDOW_FIRST and <= SDL_EVENT_WINDOW_LAST if you need to
    //     see whether it's a window event.
    case SDL_WINDOWEVENT: {
...
#ifndef PLATFORM_DESKTOP_SDL3
        // Manually detect if the window was maximized (due to SDL2 restore
        // being unreliable on some platforms) to remove the
        // FLAG_WINDOW_MAXIMIZED accordingly
        if ((CORE.Window.flags & FLAG_WINDOW_MAXIMIZED) > 0) {
          int borderTop = 0;
          int borderLeft = 0;
          int borderBottom = 0;
          int borderRight = 0;
          SDL_GetWindowBordersSize(platform.window, &borderTop, &borderLeft,
                                   &borderBottom, &borderRight);
          SDL_Rect usableBounds;
          SDL_GetDisplayUsableBounds(SDL_GetWindowDisplayIndex(platform.window),
                                     &usableBounds);

          if ((width + borderLeft + borderRight != usableBounds.w) &&
              (height + borderTop + borderBottom != usableBounds.h))
            CORE.Window.flags &= ~FLAG_WINDOW_MAXIMIZED;
        }
#endif
      } break;
      ...
      case SDL_WINDOWEVENT_RESTORED: {
        ...
        if ((SDL_GetWindowFlags(platform.window) & SDL_WINDOW_MAXIMIZED) == 0) {
          if ((CORE.Window.flags & SDL_WINDOW_MAXIMIZED) > 0)
            CORE.Window.flags &= ~SDL_WINDOW_MAXIMIZED;
        }
#endif
      } break;
    ...
    case SDL_KEYDOWN: {
#if defined(PLATFORM_DESKTOP_SDL3)
      // SDL3 Migration: The following structures have been removed: *
      // SDL_Keysym
    ...
    case SDL_TEXTINPUT: {
      // NOTE: event.text.text data comes an UTF-8 text sequence but we register
      // codepoints (int)
      ...
      if (CORE.Input.Keyboard.charPressedQueueCount < MAX_CHAR_PRESSED_QUEUE) {
        ...
        CORE.Input.Keyboard
            ...
            GetCodepointNext(event.text.text, &codepointSize);
    ...
    case SDL_JOYDEVICEADDED: {
      ...
      if (!CORE.Input.Gamepad.ready[jid] && (jid < MAX_GAMEPADS)) {
        ...
        platform.gamepadId[jid] = SDL_JoystickInstanceID(
            SDL_GameControllerGetJoystick(platform.gamepad[jid]));

        if (platform.gamepad[jid]) {
          CORE.Input.Gamepad.ready[jid] = true;
          CORE.Input.Gamepad.axisCount[jid] = SDL_JoystickNumAxes(
              SDL_GameControllerGetJoystick(platform.gamepad[jid]));
          ...
          strncpy(CORE.Input.Gamepad.name[jid],
                  SDL_GameControllerNameForIndex(jid),
                  MAX_GAMEPAD_NAME_LENGTH - 1);
          CORE.Input.Gamepad.name[jid][MAX_GAMEPAD_NAME_LENGTH - 1] = '\0';
        } else {
          TRACELOG(LOG_WARNING,
                   "PLATFORM: Unable to open game controller [ERROR: %s]",
                   SDL_GetError());
        }
      }
    } break;
      ...
      if (button >= 0) {
        for (int i = 0; i < MAX_GAMEPADS; i++) {
          if (platform.gamepadId[i] == event.jbutton.which) {
            ...
            if (CORE.Input.Gamepad.lastButtonPressed == button)
      ...
      if (axis >= 0) {
        for (int i = 0; i < MAX_GAMEPADS; i++) {
          if (platform.gamepadId[i] == event.jaxis.which) {
            // SDL axis value range is -32768 to 32767, we normalize it to
            // RayLib's -1.0 to 1.0f range
            ...
            if ((axis == GAMEPAD_AXIS_LEFT_TRIGGER) ||
                (axis == GAMEPAD_AXIS_RIGHT_TRIGGER)) {
              int button = (axis == GAMEPAD_AXIS_LEFT_TRIGGER)
...
int InitPlatform(void) {
  ...
  // NOTE: Not all systems need to be initialized, SDL_INIT_AUDIO is not
  // required, managed by miniaudio
...
#if defined(PLATFORM_DESKTOP_SDL3)
  platform.window =
      SDL_CreateWindow(CORE.Window.title, CORE.Window.screen.width,
                       CORE.Window.screen.height, flags);
#else
  ...
  // Initialize gamepads
  for (int i = 0; i < MAX_GAMEPADS; i++) {
    platform.gamepadId[i] = -1; // Set all gamepad initial instance ids as
                                // invalid to not conflict with instance id zero
  }

  for (int i = 0; (i < SDL_NumJoysticks()) && (i < MAX_GAMEPADS); i++) {
    platform.gamepad[i] = SDL_GameControllerOpen(i);
    platform.gamepadId[i] = SDL_JoystickInstanceID(
        SDL_GameControllerGetJoystick(platform.gamepad[i]));

    if (platform.gamepad[i]) {
      CORE.Input.Gamepad.ready[i] = true;
      CORE.Input.Gamepad.axisCount[i] = SDL_JoystickNumAxes(
          SDL_GameControllerGetJoystick(platform.gamepad[i]));
      ...
      strncpy(CORE.Input.Gamepad.name[i], SDL_GameControllerNameForIndex(i),
              MAX_GAMEPAD_NAME_LENGTH - 1);
  ...
  // NOTE: This is wanted because there are SDL_FINGER* events available which
  // provide unique data
  //       Due to the way PollInputEvents() and rgestures.h are currently
  //       implemented, setting this won't break SUPPORT_MOUSE_GESTURES
  ...
  CORE.Storage.basePath =
      SDL_GetBasePath(); // Alternative: GetWorkingDirectory();
  //----------------------------------------------------------------------------
